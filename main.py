...
@asynccontextmanager
async def lifespan(app: FastAPI):
    # IMPORTANT: use environment variable for Redis in production
    redis_url = "redis://localhost:6379"
    # In PXXL, this may be remote (e.g., from Render/Upstash)
    r = await redis.from_url(redis_url, encoding="utf8", decode_responses=True)
...
# root endpoint with rate limiter
@app.get("/", dependencies=[Depends(RateLimiter(times=5, seconds=60))])
async def root():
    return {"message": "Welcome to the Profile API (Production)"}
